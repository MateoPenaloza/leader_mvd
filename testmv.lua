...
function imgui.OnDrawFrame()
  ...
  local sw, sh = getScreenResolution()
   ...
   local sw, sh = getScreenResolution()
    ...
    imgui.Begin(u8'Информация | v. 1.3 beta', one_menu)
    ...
    imgui.Begin(u8'Гос. новости | v. 1.3 beta', second_menu)
	...
	imgui.End()
			...
			imgui.Begin(u8'Гос. новости | MVD | v. 1.3 beta', second_menu_1)
			...
			imgui.Begin(u8'Гос. новости | LSPD | v. 1.3 beta', second_menu_2)
			...
			imgui.SetNextWindowPos(imgui.ImVec2(sw / 2, sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.0, 0.0))
			imgui.SetNextWindowSize(imgui.ImVec2(300, 90), imgui.Cond.FirstUseEver)
			imgui.Begin(u8'Гос. новости | SFPD | v. 1.3 beta', second_menu_3)
				if imgui.Button(u8'Собеседование(3)',imgui.ImVec2(260, 20)) then
					sampSendChat('/gnews Уважаемые жители Штата! Минуточку внимания.')
			...
			imgui.SetNextWindowPos(imgui.ImVec2(sw / 2, sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.0, 0.0))
			imgui.SetNextWindowSize(imgui.ImVec2(300, 90), imgui.Cond.FirstUseEver)
			imgui.Begin(u8'Гос. новости | LVPD | v. 1.3 beta', second_menu_4)
				if imgui.Button(u8'Собеседование(3)',imgui.ImVec2(260, 20)) then
					sampSendChat('/gnews Уважаемые жители Штата! Минуточку внимания.')
    ...
    imgui.Begin(u8'Проверка работы Департаментов | v. 1.3 beta', tree_menu)
	imgui.BeginChild('left pane', imgui.ImVec2(180, 0), true)
		if not selected2 then selected2 = 1 end
        if imgui.Selectable(u8('Проверка постов городов   '), selected2 == 1) then selected2 = 1 end
        if imgui.Selectable(u8('Проверка патруля в городах'), selected2 == 2) then selected2 = 2 end
        if imgui.Selectable(u8('Статистика проверок'), selected2 == 3) then selected2 = 3 end
		imgui.EndChild()
        imgui.SameLine()
        imgui.BeginGroup()
		if selected2 == 1 then
			...
			if imgui.Button(u8'Итог проверки',imgui.ImVec2(300, 20)) then
				sampSendChat(string.format("/f Итог проверки постов: Лос-Сантос: %d/3, Сан-Фиерро: %d/3, Лас-Вентурас: %d/3", post[4], post[5], post[6]))
				local flog = io.open(path, "a")
				flog:write('['..os.date('%d.%m.%Y | %X')..'] Итог проверки постов: ЛС - '..post[4]..' СФ - '..post[5]..' ЛВ - '..post[6]..'\n')
				flog:close()
		...
		if selected2 == 2 then
			...
			if imgui.Button(u8'-##but104',imgui.ImVec2(20, 20)) then
				if post[1].v > 0 then
					post[1].v = post[1].v-1
					sampSendChat("/f Аннулировал патруль в городе Лос-Сантос. Кол-во патрулей на данный момент: "..post[1].v)
				else sampAddChatMessage("[Leader-MVD] {FFFFFF}Кол-во патрулей не может быть меньше 0.", 0x4b58ebFF) end
			end
			imgui.SameLine(nil, 5)
			if imgui.Button(u8'+##but106',imgui.ImVec2(20, 20)) then
				post[1].v = post[1].v+1
				sampSendChat("/f Обнаружен патруль в городе Лос-Сантос. Кол-во патрулей на данный момент: "..post[1].v)
			end
			imgui.SameLine(nil, 5)
			imgui.Text(u8'Лос-Сантос')
			--------------------------------
			imgui.PushItemWidth(150)
			...
			if imgui.Button(u8'-##but103',imgui.ImVec2(20, 20)) then
				if  post[2].v > 0 then
					 post[2].v =  post[2].v-1
					sampSendChat("/f Аннулировал патруль в городе Сан-Фиерро. Кол-во патрулей на данный момент: "..post[2].v)
				else sampAddChatMessage("[Leader-MVD] {FFFFFF}Кол-во патрулей не может быть меньше 0.", 0x4b58ebFF) end
			end
			imgui.SameLine(nil, 5)
			if imgui.Button(u8'+##but102',imgui.ImVec2(20, 20)) then
				 post[2].v =  post[2].v+1
				sampSendChat("/f Обнаружен патруль в городе Сан-Фиерро. Кол-во патрулей на данный момент: "..post[2].v)
			end
			imgui.SameLine(nil, 5)
			imgui.Text(u8'Сан-Фиерро')
			--------------------------------
			imgui.PushItemWidth(150)
			...
			if imgui.Button(u8'-##but100',imgui.ImVec2(20, 20)) then
				if post[3].v > 0 then
					post[3].v = post[3].v-1
					sampSendChat("/f Аннулировал патруль в городе Лас-Вентурас. Кол-во патрулей на данный момент: "..post[3].v)
				else sampAddChatMessage("[Leader-MVD] {FFFFFF}Кол-во патрулей не может быть меньше 0.", 0x4b58ebFF) end
			end
			imgui.SameLine(nil, 5)
			if imgui.Button(u8'+##but101',imgui.ImVec2(20, 20)) then
				post[3].v = post[3].v+1
				sampSendChat("/f Обнаружен патруль в городе Лас-Вентурас. Кол-во патрулей на данный момент: "..post[3].v)
			end
			imgui.SameLine(nil, 5)
			imgui.Text(u8'Лас-Вентурас')
			--------------------------------
			imgui.Text(u8'Кол-во патрулей в городах:')
			imgui.Text(u8'Лос-Сантос: '..post[1].v.. " |")
			imgui.SameLine(nil, 4)
			imgui.Text(u8'Сан-Фиерро: '..post[2].v.. " |")
			imgui.SameLine(nil, 4)
			imgui.Text(u8'Лас-Вентурас: '..post[3].v)
			if imgui.Button(u8'Итог проверки',imgui.ImVec2(300, 20)) then
				sampSendChat(string.format(" /f Итог проверки патрулей: Лос-Сантос: %s, Сан-Фиерро: %s, Лас-Вентурас: %s", post[1].v, post[2].v, post[3].v))
				local flog = io.open(path, "a")
				flog:write('['..os.date('%d.%m.%Y | %X')..'] Итог проверки патрулей: ЛС - '..post[1].v..' СФ - '..post[2].v..' ЛВ - '..post[3].v..'\n')
				flog:close()
		...
		if selected2 == 3 then
			...
			imgui.Columns(4)
			imgui.SetColumnWidth(-1, 20)
			imgui.TextQuestion(u8'Средний find')
			imgui.NextColumn()
			...
			-------------------------------------
			imgui.NextColumn()
			imgui.TextQuestion(u8'Средний % работающих сотрудников')
			imgui.NextColumn()
			...
			-----------------------------------------
			imgui.NextColumn()
			imgui.TextQuestion(u8'Среднее кол-во патрулей и занятых постов')
			imgui.NextColumn()
			...
			imgui.Separator()
			imgui.Columns(1)
			imgui.Text(u8'\t\t\t\t\t\t\tОчистить данные:')
			imgui.SameLine(nil, 3)
			imgui.TextQuestion(u8'Если в подразделении поменялся лидер, то желательно очистить данные работы подразделения  лидера. Нажмите на кнопку, на которой написан город, где поменялся лидер')
			imgui.TextQuestion(u8'Будьте внимательны, если вы нажмёте на одну из кнопок, то очистятся все данные работы подразделения')
			imgui.SameLine(nil, 1)
			if imgui.Button(u8'Лос-Сантос',imgui.ImVec2(100, 20)) then
			...
			imgui.SameLine(nil, 1)
			if imgui.Button(u8'Сан-Фиерро',imgui.ImVec2(99, 20)) then
			...
			imgui.SameLine(nil, 1)
			if imgui.Button(u8'Лас-Вентурас',imgui.ImVec2(98, 20)) then
	...
	imgui.End()
			...
			imgui.Begin(u8'Проверка работы Департаментов | Диаграммы | v. 1.3 beta', tree_menu_0)
			local it =
			{
				[1] = (mainIni.settings.sls/mainIni.settings.sfls)*100,
				[2] = (mainIni.settings.ssf/mainIni.settings.sfsf)*100,
			...
			imgui.Text(u8'Средний find:')
			imgui.RoundDiagram(flightFind, 60, 50)
			imgui.Text(u8'Среднее кол-во патрулей и занятых постов:')
			imgui.RoundDiagram(flightKpst, 60, 50)
			imgui.Text(u8'Средний % работающих сотрудников:')
			...
			imgui.Begin(u8'Проверка постов | Los-Santos | v. 1.3 beta', tree_menu_1)
			...
			imgui.SetNextWindowPos(imgui.ImVec2(sw / 2, sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.0, 0.0))
			imgui.SetNextWindowSize(imgui.ImVec2(380, 110), imgui.Cond.FirstUseEver)
			imgui.Begin(u8'Проверка постов | San-Fierro | v. 1.3 beta', tree_menu_2)
			...
			imgui.SetNextWindowPos(imgui.ImVec2(sw / 2, sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.0, 0.0))
			imgui.SetNextWindowSize(imgui.ImVec2(380, 110), imgui.Cond.FirstUseEver)
			imgui.Begin(u8'Проверка постов | Las-Venturas | v. 1.3 beta', tree_menu_3)
    ...
    imgui.Begin(u8'Сообщить в рацию | v. 1.3 beta', fo_menu)
    ...
    imgui.Begin(u8'Проверка финда | v. 1.3 beta', ears_menu)
	imgui.Text(u8'  Укажите кол-во сотрудников\n\t каждого подразделения:')
	imgui.PushItemWidth(68)
	imgui.InputInt(u8'ЛС', find[1], 1, 10)
	imgui.PopItemWidth()
	imgui.SameLine(nil, 4)
	imgui.PushItemWidth(68)
	imgui.InputInt(u8'СФ', find[2], 1, 10)
	imgui.PopItemWidth()
	imgui.PushItemWidth(68)
	imgui.InputInt(u8'ЛВ', find[3], 1, 10)
	imgui.PopItemWidth()
	imgui.SameLine(nil, 4)
	imgui.PushItemWidth(68)
	imgui.InputInt(u8'FBI', find[4], 1, 10)
	imgui.PopItemWidth()
	if imgui.Button(u8"Сохранить",imgui.ImVec2(182, 20)) then
			local flog = io.open(path, "a")
			flog:write('['..os.date('%d.%m.%Y | %X')..'] Финд организаций: ЛС - '..find[1].v..' СФ - '..find[2].v..' ЛВ - '..find[3].v..' FBI - '..find[4].v..'\n')
			flog:close()
	...
	--[[imgui.Text(u8'             Очистить данные о финдах: ')
	imgui.SameLine(nil, 1)
	imgui.TextQuestion(u8'Если в подразделении поменялся лидер, то желательно очистить данные о финдах другого лидера. Нажмите на кнопку, на которой написан город, в котором поменялся лидер')
    ...
    imgui.SetNextWindowPos(imgui.ImVec2(sw / 2, sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.0, 0.0))
    imgui.SetNextWindowSize(imgui.ImVec2(300, 130), imgui.Cond.FirstUseEver)
    imgui.Begin(u8'Поставить лидера | v. 1.3 beta', five_menu)
    ...
    imgui.Begin(u8'Поставить лидера | v. 1.3 beta', seven_menu)
		imgui.Text(u8'v. 0.1:')
		imgui.SameLine(nil, 5)
		...
		imgui.SameLine(nil, 5)
		imgui.TextQuestion(u8'- Добавлена статистика проверки постов\n-Добавлена проверка патрулей\n- Исправлены баги/ошибки')
		imgui.Text(u8'v. 0.4:')
		imgui.SameLine(nil, 5)
    ...
    imgui.Begin(u8'Обыск | v. 1.3 beta', ten_menu)
    ...
    imgui.Begin(u8'Единый процессуальный кодекс	| v. 1.3 beta', ten_menu_one)
...
function main()
	...
	if sampGetPlayerNickname(playerid) == 'Mateo_Penaloza' then
		if statusa == 0 then
			sampAddChatMessage("[Leader-MVD] {FFFFFF} Скрипт запущен.{9d00bd} Автор: Mateo Penaloza. {4b58eb} | Version: 1.4 beta", 0x4b58ebFF)
			...
			if save_rpw.v then
				...
				sampAddChatMessage("[Leader-MVD] {FFFFFF} Скрипт запущен.{9d00bd} Автор: Mateo Penaloza. {4b58eb} | Version: 1.3 beta", 0x4b58ebFF)
